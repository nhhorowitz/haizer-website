# Haizer — v5 State Export

**Single source of truth.** Every product decision, locked. Reference this when building any feature; specific sections are called out in `build-playbook.md`.
...
- **Product:** Haizer
- **Tagline:** Homes. Flips. Rentals. Direct.
- **Sub-tagline:** Buy, sell, rent, and flip — all in one place. Free to list, always.
- **Audience:** Heimishe (Orthodox Jewish) community in NY/NJ tri-state area
- **Positioning:** Premium-feeling, serious-money real estate marketplace. Direct seller-to-buyer. No commissions. No middlemen. Community-built and community-aware.
- **Competitor frame:** Free what Listco charges for. More polished than Listco. Heimishe-native unlike Zillow.

## 2. Design system

### Colors
- Brand: Emerald primary `#064E3B`, hover `#043B2C`, subtle `#D1FAE5`
- Surfaces: Page `#FAF8F3` (warm cream), Card `#FFFFFF`, Secondary `#F5F2EC`
- Text: Primary `#1A1A1A`, Secondary `#64748B`, Tertiary `#94A3B8`
- Borders: Default `#E2E8F0`, Hover `#CBD5E1`, Focus ring emerald @ 30% opacity
- Functional: Rose `#F43F5E` (save), Terracotta `#E2725B` (promoted), Success `#10B981` (Active status), Red `#DC2626` (error), Blue `#3B82F6` (info)

### Typography
- Font: Outfit (Google Fonts), weights 400/500/600
- Large-scale hierarchy. Sentence case only. No Title Case. No ALL CAPS. Minimum 14px body text.

### Geometry
- Card / panel corners: 10-12px
- Button / input corners: 6-8px
- Badge / tag corners: 999px (pill)
- Border weights: 1px default, 0.5px subtle

### Motion
- Button hover: 150ms ease, scale(1.02), soft shadow
- Button active: 100ms, scale(0.98)
- Card hover: 150ms, 1% scale, border darkens
- Listing photo on hover: 300ms zoom to 105%
- Save heart: scale 1.4 → 1 with rose color, 250ms ease-out

### Absolute rules
- No gradients on buttons
- No glows, neon, animated backgrounds
- No drop shadows except subtle hover lift
- No stock photos with people in heroes
- No emoji in UI
- 44px minimum tap targets on mobile

## 3. Tech stack (definitive)

Next.js 15 (App Router, TypeScript) · Tailwind CSS 4 · Supabase (DB/Auth/Storage/Realtime/Edge Functions) · Resend (email) · Twilio (SMS verification) · Stripe (payments) · Vercel (hosting) · Mapbox GL JS (maps) · Radix UI primitives + Tailwind (no shadcn) · Postgres FTS (search) · PostHog (analytics) · Sentry (errors) · `sharp` for image processing · HebCal API (Shabbos/YT calendar).

## 4. Database schema — core tables

- `profiles` — user data, `tags` array (`['buyer','seller','agent','investor','service_provider']`), trust score, phone_verified, email_verified, created_at, role (`user`/`admin`/`superadmin`)
- `agents` — extended profile (license_number, brokerage_name, brokerage_website, bio, specialty_tags, cities_served, store_pro_active, custom_slug, custom_banner_url, logo_url), joined to profiles
- `service_providers` — directory entries (category, business_name, services, cities_served, contact)
- `listings` — properties (listing_type, status, address fields, beds, baths, sqft, price, description, features, contact_prefs, expires_at, submitter_id, approved_at, approved_by)
- `listing_photos` — ordered photos per listing (storage_path, sort_order, is_cover)
- `listing_views` — analytics (private to owner)
- `saves` — user ↔ listing
- `saved_searches` — search criteria JSON + alert_frequency (instant/daily/weekly/off) + alert_channels
- `conversations` + `messages` — DM threads tied to a listing
- `subscriptions` — Stripe sub state (tier, status, current_period_end, customer_id)
- `promoted_listings` — Stripe one-off purchases (listing_id, expires_at)
- `ad_placements` — lender / service provider ad bookings
- `cities` — launch cities with display_name, slug, lat/lng, timezone, sort_order
- `service_categories` — Contractors, Title, etc.
- `notifications` — in-app notification feed
- `reports` — content moderation queue
- `verifications` — admin queue (license uploads, badge requests)
- `saved_calculations` — investor's saved calculator analyses

RLS policies: users can only read/write their own data; admins bypass via role check; listings public-readable when status='active'.

## 5. Auth & signup

### Signup flow
- `/signup` page with 3 buttons: Continue with Google · Continue with Apple · Continue with email
- Email path: name + email + password (with show/hide toggle)
- After account creation: route directly to unified dashboard. No role-selection screen.
- Verification email with 6-digit code sent immediately via Resend
- Welcome email sent automatically with dashboard link + first-time CTAs

### Signin flow
- `/signin` page: same 3 buttons
- Email path: email + password + Forgot password link
- Unified auth screen: tap "Sign up" in nav → land on signup; existing email auto-detected and prompts password instead

### Email verification (hybrid)
- Dashboard access immediate after signup (no gate)
- Verification required before: messaging a seller · creating a listing · claiming a Verified badge
- Persistent banner in dashboard until verified: *"Verify your email to unlock messaging and listings — Resend code →"*
- Verification codes go through immediately, even during Shabbos/Yom Tov (they expire)

### Forgot password
- `/forgot-password`: email field + "Send reset link"
- Resend sends tokenized link, 60-min expiry, single-use
- Even if email not in DB, show success UI (prevents user enumeration)
- `/reset-password?token=xxx`: new password + confirm form
- All sessions invalidated globally on reset (forces re-signin everywhere)
- Rate limit: max 3 reset requests per email per hour
- Social-login users: reset page explains *"You signed in with Google. Sign in with Google instead."*

### User type / role
- No explicit role selection at signup
- Auto-tagging system populates `profiles.tags` based on behavior:
  - First save → adds `buyer`
  - First listing created → adds `seller`
  - Agent verification submitted → adds `pending_agent`, then `agent` on approval
  - Used 3+ calculators in a session → adds `investor`
  - Service provider profile created → adds `service_provider`
- Users can hold multiple tags simultaneously
- Dashboard surfaces sections based on active tags

## 6. Nav

### Logged-out top nav
- **Left:** Haizer wordmark (emerald primary)
- **Center:** Buy · Rent · Investors · Tools · Directory · Field Notes
- **Right:** "List for free" (charcoal text link, emerald underline on hover) · **[Sign up]** (emerald primary button — only emerald in nav)
- Sticky on scroll, white background, 0.5px bottom border #E2E8F0
- Mobile: collapses to hamburger menu, wordmark stays

### Logged-in top nav
- Left + center same as above
- Right: "List for free" (text link) · Heart icon (saves count badge, top-right) · Bell icon (messages/notifications count badge) · Avatar (click → dropdown)

### Avatar dropdown (logged in)
My profile · My listings · Saved listings · Saved searches · Messages · Verification status · Billing & subscriptions · Notification settings · Become an agent *(if not yet activated)* · Help · Sign out

### Footer
- Background: secondary cream `#F5F2EC`
- Top: wordmark + tagline
- Link columns: Buy, Sell, Investors, Tools, Directory, About, Field Notes
- City quick-links: Boro Park · Williamsburg · Flatbush · Monsey · Lakewood · KJ
- Bottom thin row: Privacy · Terms · Cookies · Contact · © 2026 Haizer
- No social icons

## 7. Homepage — 12 sections

(Full prompts for Claude Design / build code in `/docs/spec/homepage.md` and the existing Claude Design prompts file already in repo.)

1. Top navigation bar
2. Hero (headline + sub + search bar with Buy/Rent/Flip toggle + 4 quick filter pills)
3. Trust strip (counts + "Free to list, always" + cities)
4. Featured homes (4-6 listing cards, horizontal carousel on mobile)
5. Browse by city (5-6 city tiles)
6. Investor opportunities (Flips column + BRRRR-ready column)
7. Featured agents & lenders strip (sponsored)
8. Free tools for investors (4 calculator cards)
9. How Haizer works (3 steps)
10. Local pros, ready when you are (9 service category tiles)
11. Sell or rent — totally free (full-width emerald CTA section)
12. Footer
...
### Layout
- Top: Greeting + personalized stats strip ("3 saved · 2 new matches · 1 unread")
- Body: adaptive cards based on `profiles.tags`
- Desktop: 60/40 two-column grid. Mobile: single column stack.

### Default cards (everyone)
1. Messages — unread count, last sender preview, "Open inbox →"
2. Saved listings — 3 thumbnail squares with price + address, "See all →"
3. Saved searches — count + new matches stat, "Manage →"
4. Recent activity — viewed listings, search history (collapsed by default)

### Tag-activated cards
- **My listings** (`seller` tag) — table with Active/Pending/Expired tabs, views, saves (private numbers), renewal banner, "Create new →" CTA, "Mark as Pending" button per row
- **Agent tools** (`agent` tag) — leads count, response rate (last 7 days), Store Pro status, Verification badge, Featured slot status
- **Investor tools** (`investor` tag) — saved calculator analyses, saved deals, cities watching, calculator shortcuts
- **Service Provider** (`service_provider` tag) — directory status, profile views, contact clicks, trial countdown

### Discover section (bottom)
- Single contextual nudge based on behavior gap:
  - No listing yet → "Have a property to sell? List free in 5 min →"
  - Saved 3+ listings → "Set up a saved search for new matches →"
  - Used 1 calculator → "Try the BRRRR analyzer →"
- One nudge at a time. Subtle background. Not pushy.

### Mobile stack order
Greeting + stats → Messages → Saved listings → Saved searches → My listings (if any) → Activity → Discover

## 9. Listings

### Search/index page
**URL:** `/listings` with query params for filters

**Top bar:**
- Search input (city/address/ZIP autocomplete via Mapbox)
- Type toggle pills with live counts: For Sale (N) · For Rent (N) · Flips (N) · All (N)
- "Save this search →" button (emerald secondary), visible once filters applied
- Sort dropdown: Most recent · Price low-high · Price high-low · Most saved · Best value (price/sqft)
- View toggle: List · Map (Mapbox)
- Result count: *"847 listings in Boro Park"*
